...
function TokenDetailsModal({
  ...
  const { data: coinGeckoPrice } = useCoinGeckoPrice(symbol);
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-brand-bg-secondary rounded-xl p-6 border border-brand-purple-900/30">
            ...
            <div className="text-4xl font-bold text-white">${formatPrice(coinGeckoPrice?.currentPrice)}</div>
            {coinGeckoPrice?.priceChangePercentage24h !== undefined && (
              <div className={`text-sm mt-1 ${coinGeckoPrice.priceChangePercentage24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {coinGeckoPrice.priceChangePercentage24h >= 0 ? '+' : ''}{coinGeckoPrice.priceChangePercentage24h.toFixed(2)}% (24h)
              </div>
            )}
...
function QuickBetCard({ token, onViewDetails }: { token: ReturnType<typeof getFeaturedTokensWithPriceFeeds>[0]; onViewDetails: () => void }) {
  ...
  const { data: coinGeckoPrice } = useCoinGeckoPrice(token.symbol);
    ...
    <div className="relative group bg-brand-bg-card border border-brand-purple-900/50 rounded-2xl shadow-lg p-6 hover:shadow-glow-purple hover:border-brand-purple-500 transition-all">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-white">${formatPrice(coinGeckoPrice?.currentPrice)}</div>
          {coinGeckoPrice?.priceChangePercentage24h !== undefined && (
            <div className={`text-xs ${coinGeckoPrice.priceChangePercentage24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {coinGeckoPrice.priceChangePercentage24h >= 0 ? '+' : ''}{coinGeckoPrice.priceChangePercentage24h.toFixed(2)}%
            </div>
          )}
        </div>
      </div>
...
function HistoryPredictionCard({ predictionId }: { predictionId: number }) {
  ...
  if (userBets && Array.isArray(userBets) && userBets.length > 0 && timeframe) {
    ...
    if (totalBetAmountUSD > 0) {
      pointsEarned = calculatePoints(totalBetAmountUSD, Number(timeframe), userWon ?? false);
    }
  }
...
function ActivePredictionCard({ predictionId }: { predictionId: number }) {
  ...
  let totalYesPool: bigint | undefined;
  let totalNoPool: bigint | undefined;
  let resolved: boolean | undefined;
  let winningOutcome: boolean | undefined;
  let actualEndPrice: bigint | undefined;
        ...
        <div className="flex-1">
          ...
          <div className="text-sm text-gray-400">
            {!resolved && !isExpired ? (
              ...
              <span>Ended {endTime ? formatDate(endTime) : 'N/A'}</span>
            )}
          </div>
        </div>
      </div>
